    ...
    for epoch in progressbar:
        if epoch > 0:
            ...
            for bb, (x, y_B) in enumerate(tr_loader):
                ...
                loss_xent = xent_loss_func(logits_BC, y_B.to(device), reduction='mean')
                ...
                loss_l2 = torch.sum(model.trainable_params['output.weight'] ** 2)
        ...
        with torch.no_grad():
            ...
            for xva_B3HW, yva_B in va_loader:
                ...
                va_xent += xent_loss_func(logits_BC, yva_B.to(device), reduction='sum').item() / n_valid
